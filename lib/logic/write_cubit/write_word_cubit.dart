...
class WriteCubit extends Cubit<WriteCubitState> {
  ...
  void addExample(int wordIndex) {
    _tryAndCatchBlock(() {
      List<Word> words = getWordsFromDatabase();
      words[wordIndex] = words[wordIndex].addExapmle(inputtext, isarabic);
      box.put(savedwords, words);
    });
  }

  void deleteExample(int wordIndex, int exampleIndex, isArabicExample) {
    _tryAndCatchBlock(() {
      List<Word> words = getWordsFromDatabase();
      words[wordIndex] =
          words[wordIndex].deleteExapmle(exampleIndex, isArabicExample);
      box.put(savedwords, words);
    });
  }
